...
{//printf("a");
    ...
    //BA(4);
    pso = fopen("fipso1-4 -uring.txt", "a+");
                        ...
                        for(vgen = 0; vgen < gennum; vgen++)            //gennum代进化；
                        {
                            for(i = 0; i < dim + 1; i++)
                            {
                                for(j = 0; j < N; j++)
                                {
                                    if(i < dim)
                                    {
                                        float numerator=0,r1,r2;

                                        r1=(double)rand() / RAND_MAX * (c1+c2) ;
                                        r2=(double)rand() / RAND_MAX * (c1+c2) ;

                                        if(j==0)
                                            numerator += r1*(pbest[(dim+1)*1+i]-coor[(dim+1)*j+i])+r2*(pbest[(dim+1)*(N-1)+i]-coor[(dim+1)*j+i]);
                                        else if(j==N-1)
                                            numerator += r1*(pbest[i]-coor[(dim+1)*j+i])+r2*(pbest[(dim+1)*(N-2)+i]-coor[(dim+1)*j+i]);
                                        else
                                            numerator += r1*(pbest[(dim+1)*(j-1)+i]-coor[(dim+1)*j+i])+r2*(pbest[(dim+1)*(j+1)+i]-coor[(dim+1)*j+i]);
                                        pm[(dim+1)*j+i] = numerator/(r1+r2);



                                        vel[(dim+1)*j+i] = x * ( vel[(dim+1)*j+i] + (c1+c2) * pm[(dim+1)*j+i] );
