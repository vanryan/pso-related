...
// vlneighs:The number of neighbors a paricle learns from (self-excluded)
    ...
    pso = fopen("lipso2.0-all-f4.txt", "a+");
...
*/
    ...
    pso = fopen("lipso2.0-all-f4-SucEndFit.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%1.30f\n",i+1,Sto_SucEndFit[i]);
    fclose(pso);
    pso = fopen("lipso2.0-all-f4-SucGens.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%f\n",i+1,Sto_SucGens[i]);
    fclose(pso);
    pso = fopen("lipso2.0-all-f4-SucRate.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%f\n",i+1,Sto_SucRate[i]);
    fclose(pso);
}
