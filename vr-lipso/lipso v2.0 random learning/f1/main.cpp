...
// vlneighs:The number of neighbors a paricle learns from (self-excluded)
    ...
    pso = fopen("lipso2.0-all-f1.txt", "a+");
    ...
    {
        printf("Beginning of vlneighs:%d\n",vlneighs);
...
*/
                ...
                printf("End of initialization for vtest %d of vfun %d\n",vtest,vfun);
                for(vgen = 0; vgen < gennum; vgen++)
                {

                    for(i=0; i<N; i++)
                        randlneighs[i]=-1; // initializing the indices of learnees

                    choose_rand_neighbors(vlneighs);

                    //for(i=0; i<N; i++)
                        //printf("%d",randlneighs[i]); // initializing the indices of learnees
                    //if(vlneighs==2)
                        //printf("chose\n");

                    get_vel_update(vlneighs);

                    //if(vlneighs==2) printf("update; vgen:%d\n",vgen);
        ...
        printf("End of vlneighs:%d\n\n",vlneighs);
    } // End of for vlneighs!

    fclose(pso);

    pso = fopen("lipso2.0-all-f1-SucEndFit.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%1.30f\n",i+1,Sto_SucEndFit[i]);
    fclose(pso);
    pso = fopen("lipso2.0-all-f1-SucGens.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%f\n",i+1,Sto_SucGens[i]);
    fclose(pso);
    pso = fopen("lipso2.0-all-f1-SucRate.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%f\n",i+1,Sto_SucRate[i]);
    fclose(pso);
}
