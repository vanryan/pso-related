    ...
    for(j = 0; j < N; j++)
    {
        ...
        for(i = 0; i < dim; i++)
        {
            ...
            if(con_neighs_counter>=lneighs)
            {
                ...
                var_phi = sum_coef * phi / (lneighs+1);
            }
            else  // con_neighs_counter<lneighs
            {
                for( k =0; k < con_neighs_counter; k++) // One particle can only learn from a certain number of neighbors
                {
                    coef[(int)PBNINDEX[1][k]] = (double)rand() / RAND_MAX;
                    sum_coef += coef[(int)PBNINDEX[1][k]];
                    pos_ave += coef[(int)PBNINDEX[1][k]] * pbest[(dim+1)*((int)PBNINDEX[1][k])+i];
                }
                var_phi = sum_coef * phi / (con_neighs_counter+1);
            }
            pos_ave /= sum_coef;
