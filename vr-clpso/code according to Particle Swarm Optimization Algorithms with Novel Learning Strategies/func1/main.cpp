...
int main()
{
    ...
    */
    ...
    pso = fopen("vrclpso.txt", "a+");  // We cannot use 'w' on the server in the lab in lack of privileges.
            ...
            for(i = 0; i < dim + 1; i++)
            {
                if(refgap_flag == 0 )
                {
                    for(ite_a=0; ite_a<N; ite_a++)
                    {
                        ...
                        if(rand_p<=Prob_c)
                            cl_rec[ite_a]= ceil(rand() / RAND_MAX * N) - 1; // Get a integer number from 0 to N-1
                        else
                            cl_rec[ite_a] = ite_a;
