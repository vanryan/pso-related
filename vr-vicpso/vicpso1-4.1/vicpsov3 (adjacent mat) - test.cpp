...
//const int N = 15;
const int N = 3; // Population; This will cause a waste of ram
const int testnum = 2; // Running times for experiments
const int gennum = 1;  // Number of generations
            ...
            for(varc2 = 0; varc2 < 1 ; varc2++)
            {
                for(vfun = 1; vfun < 2; vfun++)
                {
                    ...
                    for(vtest = 0; vtest < testnum; vtest++)  // Run the experiment
                    {
                        ...
                        for(i = 0; i < dim + 1; i++)
                        {
                            for(j = 0; j < N; j++)
                            {
                                ...
                                { // i=dim
                                    ...
                                    // Initializing pbest
                                    pbest[(dim+1)*j+dim] = coor[(dim+1)*j+dim];
                                    gbest[(dim+1)*j+dim] = coor[(dim+1)*j+dim];
                                    /*
                        ...
                        for(j=0;j<N;j++)
                            for(k=j;k<N;k++) // Avoid Relapse
                            {
                                int dist=getDistance(j,k);
                                if( dist <= absRadius && dist > 0) // k Can be perceived by j
                                    {
                                        adjmat[j][k]=1;
                                        adjmat[k][j]=1;
                                    }
                        ...
                        printf("\nHere goes the adjacent matrix:[");
                        for(j=0;j<N;j++){
                            for(k=0;k<N;k++){
                                if(k==N-1) printf("%d",adjmat[j][k]);
                                else printf("%d,",adjmat[j][k]);
                            }
                        ...
                        for(j=0;j<N;j++){
                             for(k=0;k<N;k++)
                                if( adjmat[j][k] == 1 && adjmat[k][j] == 1)// k Can be perceived by j
