...
//const int N = 15;
const int N = 60; // Population; This will cause a waste of ram
...
for(relRadius=0.1;relRadius<=1.3;relRadius+=0.1){ // Loop layer 1 (relRadius)
        ...
        for(varc1 = 0; varc1 < 1; varc1++)
        {
            ...
            for(varc2 = 0; varc2 < 1 ; varc2++)
            {
                for(vfun = 1; vfun < 5; vfun++)
                {
                                ...
                                for( j=0; j<N ;j++ )
                                    for( k=0; k<N ;k++ )
                                        if( adjmat[j][k] != 0) // k Can be perceived by j
                                            aveDegree+=1;
                                aveDegree/=N; // Got it
                                AveDeg[radRec*4+vfun][vgen+1] += aveDegree; // Storing Average Degree of this particular generation
                            }
                    ...
                    AveOptGen[radRec*4+vfun]=avggennew;
                    AveOptRate[radRec*4+vfun]=(double)ngoal / testnum * 100;
...
// Get da data in da file!
    ...
    for(i=0;i<13;i++){
        fprintf(pso, "%f,%f,%f\n",0.2+i*0.1,AveOptGen[i*4],AveOptRate[i*4]);
    }
    fclose(pso);
...
j*10,AveDeg[0][j],j*10,AveDeg[4][j],j*10,AveDeg[8][j],j*10,AveDeg[12][j],j*10,AveDeg[16][j],j*10,AveDeg[20][j],j*10,AveDeg[24][j],j*10,AveDeg[28][j],j*10,AveDeg[32][j],j*10,AveDeg[36][j],j*10,AveDeg[40][j],j*10,AveDeg[44][j],j*10,AveDeg[48][j]);
    ...
    for(i=0;i<13;i++){
        fprintf(pso, "%f,%f,%f\n",0.2+i*0.1,AveOptGen[i*4+1],AveOptRate[i*4+1]);
    }
    fclose(pso);
    ...
    fprintf(pso, "RelRadius,AveOptGen,AveOptRate,AveDeg\n");
    for(i=0;i<13;i++){
        fprintf(pso, "%f,%f,%f\n",0.2+i*0.1,AveOptGen[i*4+2],AveOptRate[i*4+2]);
    }
    fclose(pso);
    ...
    fprintf(pso, "RelRadius,AveOptGen,AveOptRate,AveDeg\n");
    for(i=0;i<13;i++){
        fprintf(pso, "%f,%f,%f\n",0.2+i*0.1,AveOptGen[i*4+3],AveOptRate[i*4+3]);
    }
    fclose(pso);
