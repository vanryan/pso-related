...
from 0.1-1.4 stepped by interval 0.02
...
//const int N = 15;
const int N = 30; // Population; This will cause a waste of ram
...
int main()
{
    ...
    char buffer[15*14*5]; // A buffer to write the file, 15 is the estimated char numbers
    char storage[19*14*5];
...
//When the time comes they got loaded into the file, we'll have them become real 'average'
    ...
    float AveOptGen[2*4];  // FuncNumber*RadiusNumber: Every 19[radius] with a func
    float AveOptRate[8];  // FuncNumber*RadiusNumber: Every 19[radius] with a func
    float AveDeg[8][gennum/aveDegInterval+1];  // [FuncNumber*RadiusNumber][Every 10 gen]
    ...
    for(relRadius=0.2; relRadius<0.4; relRadius+=0.1) // Loop layer 1 (relRadius)
    {
        printf("a1");
...
// Get da data in da file!
    ...
    for(i=0; i<2; i++)
    {
        fprintf(pso, "%f,%f,%f\n",0.1+i*0.02,AveOptGen[i*4],AveOptRate[i*4]);
    }
    fclose(pso);
    ...
    {
        int inIte;
        for(inIte=0; inIte<2; inIte++)
        {
            fprintf(pso, "%d,%f,",j*10,AveDeg[4*inIte][j]);
        }
        fprintf(pso, "\n" );
    }
    ...
    for(i=0; i<2; i++)
    {
        fprintf(pso, "%f,%f,%f\n",0.1+i*0.02,AveOptGen[i*4+1],AveOptRate[i*4+1]);
    }
    fclose(pso);
    ...
    for(j=0; j<=(gennum/aveDegInterval); j++)
    {
        ...
        for(inIte=0; inIte<2; inIte++)
        {
            fprintf(pso, "%d,%f,",j*10,AveDeg[1+4*inIte][j]);
        }
        fprintf(pso, "\n");
    }
    ...
    for(i=0; i<2; i++)
    {
        fprintf(pso, "%f,%f,%f\n",0.1+i*0.02,AveOptGen[i*4+2],AveOptRate[i*4+2]);
    }
    fclose(pso);
    ...
    for(j=0; j<=(gennum/aveDegInterval); j++)
    {
        ...
        for(inIte=0; inIte<2; inIte++)
        {
            fprintf(pso, "%d,%f,",j*10,AveDeg[2+4*inIte][j]);
        }
        fprintf(pso, "\n");
    }
    ...
    for(i=0; i<2; i++)
    {
        fprintf(pso, "%f,%f,%f\n",0.1+i*0.02,AveOptGen[i*4+3],AveOptRate[i*4+3]);
    }
    fclose(pso);
    ...
    for(j=0; j<=(gennum/aveDegInterval); j++)
    {
        ...
        for(inIte=0; inIte<2; inIte++)
        {
            fprintf(pso, "%d,%f,",j*10,AveDeg[3+4*inIte][j]);
        }
        fprintf(pso, "\n");
    }
