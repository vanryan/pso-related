...
const int testnum = 4; // Running times for experiments
...
int main()
{
    ...
    int i, j, k, l, varw, varc1, varc2, vfun=2, vtest, vgen, temp; // iteration vars
...
//When the time comes they got loaded into the file, we'll have them become real 'average'
    ...
    float AveOptGen[14*5];  // FuncNumber*RadiusNumber: Every 19[radius] with a func
    float AveOptRate[70];  // FuncNumber*RadiusNumber: Every 19[radius] with a func
    float AveDeg[70][gennum/aveDegInterval+1];  // [FuncNumber*RadiusNumber][Every 10 gen]
                                            //So the truth is, we're getting the average(a matrix) of average(a test)
    float AveOptFit[70][gennum/aveDegInterval+1]; // The average of the gbest fitness in the end of each test (last generation)
...
for(relRadius=0.3;relRadius<1.41;relRadius+=0.02){ // Loop layer 1 (relRadius)
...
// Get da data in da file!
    ...
    for(i=1;i<71;i++){
        fprintf(pso, "%f,%f,%f\n",0.1+i*0.02,AveOptGen[i],AveOptRate[i]);
    }
    fclose(pso);
    ...
    pso = fopen("vicpsoFunc2-OptFit.txt", "w");
    fprintf(pso, "%s\n",storage );
