...
const int changeInte = 200; // The interval we change the perceptive radius
...
int main()
{
    ...
    int i, j, k, l, varw, varc1, varc2, vfun=4, vtest, vgen; // iteration vars
                        ...
                        */
                        ...
                        for(vgen = 0; vgen < gennum; vgen++)         // Evolution starts
                        {
                            ...
                            if( (vgen+1) % (changeInte) == 0 && vgen!=0)
                            {
                                ...
                                //Forgetting
                                for(iteraa=0;iteraa<N;iteraa++)
                                    for(iterab=0; iterab<=dim; iterab++){
                                        gbest[(dim+1)*iteraa+iterab] = coor[(dim+1)*iteraa+iterab];
                                        pbest[(dim+1)*iteraa+iterab] = coor[(dim+1)*iteraa+iterab];
                                    }
...
// Get da data in da file!
    ...
    pso = fopen("c:/pso/vicpsov4.4.3/200/vicpsoFunc4-main.txt", "w");
    ...
    pso = fopen("c:/pso/vicpsov4.4.3/200/vicpsoFunc4-OptFit.txt", "w");
    fprintf(pso, "%s\n",storage );
    ...
    pso = fopen("c:/pso/vicpsov4.4.3/200/vicpsoFunc4-EndSucFit.txt","w");
    for(i=0; i<relRange;i++){
