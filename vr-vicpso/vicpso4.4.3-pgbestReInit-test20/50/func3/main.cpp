...
const int changeInte = 50; // The interval we change the perceptive radius
...
// Get da data in da file!
    ...
    pso = fopen("c:/pso/vicpsov4.4.3/50/vicpsoFunc3-main.txt", "w");
    ...
    pso = fopen("c:/pso/vicpsov4.4.3/50/vicpsoFunc3-OptFit.txt", "w");
    fprintf(pso, "%s\n",storage );
    ...
    pso = fopen("c:/pso/vicpsov4.4.3/50/vicpsoFunc3-EndSucFit.txt","w");
    for(i=0; i<relRange;i++){
