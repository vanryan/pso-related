...
// AveEndFit stores the ending fitnesses of only the successful tests, meanwhile AveAllEndFit stores ending fitnesses regardless of success or failure
    ...
    pso = fopen("../../lipso1.1-rec-all-cf1.txt", "a+");
    ...
    pso = fopen("../../lipso1.1-all-cf1.txt", "a+");

    x = 0.729;

    for(vlneighs =1; vlneighs< N; vlneighs++)
    {
        stdindex[vlneighs-1]=0;
...
*/
            ...
            fprintf(pso, "平均代数为,%f,达优率,%f%%,LastSucGb,%1.40f,LastAllGb,%1.40f,StdIndex,%f\n", avggennew, (double)sucCounter / testnum * 100, AveEndFit[vfun-1]/sucCounter,AveAllEndFit[vfun-1]/testnum, stdindex[vlneighs-1]);

        } // End of for VFUN!
        printf(">>>>>End of running vlneighs:%d\n\n",vlneighs);
    } // End of for vlneighs!
