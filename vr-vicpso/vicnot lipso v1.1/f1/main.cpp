...
// vlneighs:The number of neighbors a paricle learns from (self-excluded)
    ...
    pso = fopen("../../fipso1.1-all-f1.txt", "a+");
...
*/
            ...
            printf("\nAveGen,%f,SucRate,%f,SucEndFit,%1.30f,LastFit,%1.30f,StdIndex,%f\n", avggennew, (double)sucCounter/testnum*100,Sto_SucEndFit[vlneighs-1],AveAllFit[vfun-1]/testnum,stdindex[vlneighs-1]);
    ...
    pso = fopen("fipso1.1-all-f1-SucEndFit.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%1.30f\n",i+1,Sto_SucEndFit[i]);
    fclose(pso);
    pso = fopen("fipso1.1-all-f1-SucGens.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%f\n",i+1,Sto_SucGens[i]);
    fclose(pso);
    pso = fopen("fipso1.1-all-f1-SucRate.txt", "a+");
    for(i=0;i<N;i++)
        fprintf(pso,"%d,%f\n",i+1,Sto_SucRate[i]);
    fclose(pso);
}
